 ...
 */
#[Block(
  id: "hp_near_end_free_movies_block",
  admin_label: new TranslatableMarkup("Films gratuits qui expirent bientôt"),
  category: new TranslatableMarkup("Audiodescription")
)]
class HpNearEndFreeMoviesBlock extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  public function build() {
    ...
    $title = $homepage->get('field_near_end_movies_title')->value;

    $countFreeMovies = $this->countMoviesWithFreeSolution();
    $cta = [
      'url' => '/recherche?is_free=1',
      'text' => 'Découvrir les ' . $countFreeMovies . ' films gratuits',
      'target' => 'self',
      'external' => FALSE,
      'style' => 'primary',
    ];

    $moviesIds = $this->nearEndMoviesWithFreeSolution();
    ...
    return [
      '#theme' => 'hp_near_end_free_movies_block',
      '#title' => $title,
  ...
  private function nearEndMoviesWithFreeSolution():array {
    ...
    $sql = <<<SQL
      WITH base AS (
          SELECT DISTINCT
              m.nid                                         AS movie_id,
              er.field_pg_end_rights_value::date            AS end_rights,
          ...
          FROM node_field_data              AS m
              ...
              -- Champs partenaires
                   LEFT JOIN paragraph__field_pg_end_rights   AS er ON er.entity_id = partner_paragraph.id
          ...
          WHERE m.type = 'movie'
            ...
            AND er.field_pg_end_rights_value IS NOT NULL
            AND er.field_pg_end_rights_value::date > current_date
      ),
           ranked AS (
               SELECT
                   b.movie_id,
                   ...
                   ROW_NUMBER() OVER (
                       ...
                       ORDER BY
                           ...
                           b.end_rights   ASC NULLS LAST,
                           b.partner_name,
                           b.link_uri
                       ) AS rn
               FROM base b
           )
      SELECT movie_id, end_rights
      FROM ranked
      WHERE rn = 1
      ORDER BY end_rights ASC, movie_id
      LIMIT :limit;
      SQL;
